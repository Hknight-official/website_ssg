...
function Home(){
    ...
    return (
        <>
            {
                ((!userData['name']) ? (<div>Loading please wait...</div>) : (
                    <DataUserContext.Provider value={userData}>
                        <div className="home-page row">
                            <LeftNavBar onChooseUser={onChooseClient} listSupporter={listSupporter} handleClickLeft={() => {setIsLeft(!isLeft)}} isLeft={isLeft} />
                            <Chat listSupporter={listSupporter} handleSetListSupporter={setListSupporter} handleClickLeft={() => {setIsLeft(!isLeft)}} handleClickRight={() => {setIsRight(!isRight)}} roomId={roomId} />
